...
AddEventHandler('esx_society:withdrawMoney2', function(societyName, amount)
	...
	if xPlayer.job2.name == society.name then
		...
		print(('esx_society: %s attempted to call withdrawMoney2!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_society:depositMoney2', function(societyName, amount)
	...
	if xPlayer.job2.name == society.name then
		...
		print(('esx_society: %s attempted to call depositMoney2!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_society:washMoney2', function(society, amount)
	...
	if xPlayer.job2.name == society then
		...
		print(('esx_society: %s attempted to call washMoney2!'):format(xPlayer.identifier))
	end
end)
...
ESX.RegisterServerCallback('esx_society:getEmployees2', function(source, cb, society)
	...
	local query = "SELECT identifier, job2_grade FROM `users` WHERE `job2`=@job2 ORDER BY job2_grade DESC"

	if Config.EnableESXIdentity then
		query = "SELECT identifier, job2_grade, firstname, lastname FROM `users` WHERE `job2`=@job2 ORDER BY job2_grade DESC"
	end
